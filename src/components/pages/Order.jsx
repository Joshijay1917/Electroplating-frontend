...
const Order = () => {
    ...
    const handleCustomer = (e) => {
        if(e.target.value) {
            ...
            setcurrentCus(data.customers.filter(c => data.orders.find(o => o.customerid === c._id)).filter(c => c.name.includes(e.target.value.toLocaleLowerCase())))
        } else {
            setcurrentOrder(data.orders)
        ...
        <div className='setheight pb-[68px]'>
            ...
            {currentOrder.length !== 0
                ? <div className='mx-5 my-8 shadow-2xl rounded-2xl'>
                    ...
                    {currentCus.length !== 0
                          ? currentCus.map(c => {
                            return <div key={c._id} className='mx-2 flex flex-col my-3 shadow-2xl border text-gray-700 dark:text-white border-gray-400 rounded-2xl p-3'>
                            <div className='flex items-center'>
                            ...
                            {currentOrder.filter(o => o.customerid === c._id).map(o => {
                                return <div className={`rounded-xl flex justify-between items-center px-5 mt-3 relative overflow-hidden text-gray-700 dark:text-white shadow-gray-500 transition-all hover:-translate-y-0.5 hover:shadow-lg dark:bg-gray-700 dark:border-gray-600 bg-gray-50 border-gray-400 border`}>
                          ...
                          :<div className='flex justify-center text-gray-700 dark:text-white items-center'>
                ...
                : <div className='flex justify-center text-gray-700 dark:text-white items-center m-8'>
                    <BiUser className="text-6xl text-blue-600 dark:text-blue-400 mr-3" />
                    <div>
